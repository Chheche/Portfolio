...
function hideName(e) {
    document.getElementById(e + 'Name').style.opacity = '0';
}

document.getElementById('telecharger').addEventListener('click', function() {
    var cheminCV = 'CV-Rafael-BARRETO.pdf';

    fetch(cheminCV)
        .then(response => response.blob())
        .then(blobCV => {
            var a = document.createElement('a');
            a.href = window.URL.createObjectURL(blobCV);
            a.download = 'CV-Rafael-BARRETO.pdf';
...
const element = document.querySelector('.ztext');
new Ztextify(".ztext", {
    depth: "20px",
    layers: 8,
    fade: true,
